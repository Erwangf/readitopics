...
public class MyHLDA {
	...
	*/
    ...
    /*private void myprint(NCRPNode node, int indent, boolean withWeight) {
    ...
    public ArrayList<String> getNodeForExport(boolean withWeight) {
    	ArrayList<String> node_list = new ArrayList<>();
		getNodeForExport(rootNode, node_list, withWeight);
		return node_list;
    }
    
    public void getNodeForExport(NCRPNode node, ArrayList<String> node_list, boolean withWeight) {
 		StringBuffer out = new StringBuffer();
 		out.append(node.totalTokens + "\t" + node.customers + "\t");
 		out.append(node.getTopWords(numWordsToDisplay, withWeight));
 		node_list.add(out.toString());
 		for (NCRPNode child: node.children) {
 			getNodeForExport(child, node_list, withWeight);
 		}
     }
    ...
    private int countLeaves(NCRPNode node)
    {
